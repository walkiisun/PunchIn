...
if (isset($_POST['clock_in'])) {
    $sqlCheck = "SELECT COUNT(*) FROM Work_Time_Instance WHERE user_id = ? AND Clock_OUT_Timestamp IS NULL";
    $stmtCheck = $conn->prepare($sqlCheck);
    $stmtCheck->bind_param("i", $userId);
    $stmtCheck->execute();
    $stmtCheck->bind_result($activeClockInCount);
    $stmtCheck->fetch();
    $stmtCheck->close();

    if ($activeClockInCount > 0) {
        $_SESSION['message'] = "<p style='color:red;'>You are already clocked in! Please clock out before clocking in again.</p>";
    } else {
        $clockInTime = date("Y-m-d H:i:s");
        $workTimeId = getNextWorkTimeId($conn, $userId);

        $sqlInsert = "INSERT INTO Work_Time_Instance (work_time_id, user_id, Clock_IN_TimeStamp) VALUES (?, ?, ?)";
        $stmtInsert = $conn->prepare($sqlInsert);
        $stmtInsert->bind_param("iis", $workTimeId, $userId, $clockInTime);
...
<main>
    ...
    <div class="dash"><a href=""><button>View Dashboard</button></a></div>
