...
CREATE TABLE IF NOT EXISTS `mydb`.`EmployeeUser` (
  ...
  `idAdminUser` INT NOT NULL COMMENT 'To bind a employee to a admin',
  PRIMARY KEY (`User_ID`),
  INDEX `fk_Employee_Position1_idx` (`Position_Position_ID` ASC) VISIBLE,
  INDEX `fk_EmployeeUser_AdminUser1_idx` (`Account_Creation_Approval_Flag` ASC, `idAdminUser` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_EmployeeUser_AdminUser1`
    FOREIGN KEY (`Account_Creation_Approval_Flag` , `idAdminUser`)
    REFERENCES `mydb`.`AdminUser` (`idAdminUser` , `idAdminUser`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
